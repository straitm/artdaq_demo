...
if [ $? -eq 0 ]; then
    ...
    $git_working_path/tools/xt_cmd.sh $root --geom '132x33 -sl 2500' \
        -c '. ./setupARTDAQDEMO' \
        -c examples/udpReceiver/start3x2x2System.sh
    sleep 2

    $git_working_path/tools/xt_cmd.sh $root --geom 132 \
        -c '. ./setupARTDAQDEMO' \
        -c ':,sleep 10' \
        -c 'examples/triggeredReceiver/manage3x2x2System.sh init' \
        -c ':,sleep 5' \
        -c 'examples/triggeredReceiver/manage3x2x2System.sh -N 201 start' \
        -c ':,sleep 30' \
        -c 'examples/triggeredReceiver/manage3x2x2System.sh stop' \
        -c ':,sleep 5' \
        -c 'examples/triggeredReceiver/manage3x2x2System.sh shutdown' \
        -c ': For additional commands, see output from: manage3x2x2System.sh --help' \
        -c ': To see the output from the demo, run' \
        -c ': art -c readRoot.fcl /tmp/artdaqdemo_r000201_sr01_*.root' \
        -c ':: manage3x2x2System.sh --help' \
        -c ':: manage3x2x2System.sh exit'
else
